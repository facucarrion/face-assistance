...
from lib.temp_images.crud import create_empty_temp_image, delete_temp_image, get_temp_image_person_by_device, get_temp_image, confirm_temp_image
...
@temp_images_router.put("/{id_temp_images}", response_model=TempImagesBase)
async def confirm_temp_image_route(id_temp_images: int, db = Depends(get_db)):
  temp_image = confirm_temp_image(db, id_temp_images)
  return temp_image
